...
import { StyledActiveSideBar } from "../styledComponents/SideBar";
...
export default function ActiveSideBar() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // initialId 상태를 초기 값으로 복구
      setInitialId(id);
    };
  }, []); // 빈 배열을 전달하여 컴포넌트가 언마운트될 때만 실행

  // 현재 경로를 추출합니다.
  const currentPath = location.pathname;

  // 내비게이션 메뉴 항목과 경로를 매칭하고, 현재 경로와 일치하는지 확인합니다.
  const isProjectActive = currentPath === `/${initialId}`;
  const isKanbanActive = currentPath === `/${initialId}/kanban`;
  const isIssueActive = currentPath === `/${initialId}/issue`;
  ...
  return (
    <StyledActiveSideBar>
      ...
      <nav className="menus">
        ...
        <button
          onClick={() => navigate(`/${initialId}/kanban`)}
        ...
        <button
          onClick={() => navigate(`/${initialId}/issue`)}
